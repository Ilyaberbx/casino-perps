...
function buildPortfolio(snapshot: PortfolioSnapshot): PortfolioReader {
  return {
...
function buildBalances(rows: ReadonlyArray<Balance>): BalancesReader {
  return {
...
const placeholderValidation = buildFakeOrderValidation({ symbol: 'BTC-PERP', markPrice: 0, availableMargin: 0 })

function buildVenue(placeOrderSpy: Trader['placeOrder'], options: VenueOptions = {}): Venue {
  const positions = options.positions
  const validation = placeholderValidation
  // Order entry now reads Available-to-Trade / spot sizing / Current-Position
  // from the ACTING account group (`ownAccount`, ADR-0038). With no spectate
  // concept in these tests acting === viewing, so the same reader fakes back
  // both the viewing capabilities (read by `withMarketContext`'s validation
  // pricing) and the `ownAccount` group (read by the hook).
...
// A flat position (size 0) for BTC-PERP that only carries leverage — used to
// seed the order-entry leverage (via `useLeverageMargin`) for margin-unit tests.
function buildPosition(leverage: number): PerpPositionSnapshot {
  return {
    symbol: 'BTC-PERP',
    side: 'long',
    size: 0,
...
function buildSpotBalance(asset: string, available: number): Balance {
...
const DEFAULT_PERP_MARKET: Market = {
  symbol: 'BTC-PERP',
  baseAsset: 'BTC',
  quoteAsset: 'USD',
  venue: 'mock',
...
function withMarketContext(venue: Venue, markPrice: number, market: Market): Venue {
  ...
  return {
    ...
    capabilities: {
      ...venue.capabilities,
      trader: { ...trader, validateDraft: validation.validateDraft, previewOrder: validation.previewOrder },
    },
  }
}
