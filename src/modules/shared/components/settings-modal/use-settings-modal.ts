...
import { useTradingMode } from '../../providers/trading-mode-provider'
 ...
 * accent color), and the global Trading Mode (`useTradingMode`), and projects the
 * nav rail + each section's data. The Appearance section owns the Theme switch +
 * Accent Color picker; the Trading section owns the Pro/Simple toggle. Dumb
 * components below consume only prop slices.
 */
export function useSettingsModal(): SettingsModalContent {
  ...
  return {
    ...
    tradingMode,
    onSelectTradingMode: setTradingMode,
  }
}
