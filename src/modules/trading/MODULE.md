...
- `LeverageMarginProvider` — owns the order form's leverage + margin-mode state for the selected market as **shared** state (`useLeverageMargin()`). `applyLeverage` signs the venue `setLeverage`. Seeded from the Acting `perpsPositionsSnapshot`, falling back to 1×/cross. Mounted inline by `SimpleOrderTicket` (perps only).
...
- `FavoritesProvider` — persisted favorites set (`useFavorites()` / `useFavoritesOptional()`). Mounted **app-wide** in `AccountSessionRoot`: the market list, the search overlay, the trade page's market strip, and the lobby's `/?view=favorites` grid all share one store.
