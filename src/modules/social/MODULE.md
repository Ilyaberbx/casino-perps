...
- `LiveWinsTicker` — the horizontally scrolling LIVE WINS strip of small market/username/green-$ cards. Self-contained (owns state via `useLiveWins`), takes no props. The marquee duplicates the card list for a seamless loop.
- `DISCLOSURE_TEXT` — the string `"Chat and Live Wins are simulated."`, exported for a **later phase** to render as a visible footer/near-surface disclosure (PRD §13 R5). This module does not render it itself.
- `ChatMessage`, `LiveWin` — the message-union and win row types, for a consumer that wants to type against the reel.

The smart hooks (`useChat`, `useLiveWins`) and all sub-components are **private** — consumers mount `ChatPanel` / `LiveWinsTicker` and get the behavior for free.

## Owns

- The entire **chat + live-wins fixture data set** (`social.fixtures.ts`): fake users and their name colors, the seed messages, the looping scripted reel, and the live-wins seed. This is the single file to edit to change what the crowd "says".
...
- The **live-wins rotation timer** (`LIVE_WINS_ROTATE_INTERVAL_MS`) that rotates the fixed-length window so a fresh card scrolls in without the list growing.
...
- The casino palette CSS variables (`--bg-*`, `--accent-*`, `--win`, `--text-*`) with **hard hex fallbacks** (`var(--accent-action, #ce37dc)`), so the module renders correctly whether or not the design-tokens phase has defined them yet.
...
- **These are simulated wins next to a real-money button.** `social.fixtures.ts` carries a standing header comment to this effect, and `DISCLOSURE_TEXT` exists so the admission is rendered somewhere visible. Do not remove either without replacing the disclosure.
...
- Mounting the panel/ticker into the three-column shell and mobile "Chat" tab — the `app/` shell phase wires these in.
